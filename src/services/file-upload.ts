...
const upload = multer({
  storage: multerS3({
    ...
    bucket: (req: any, file: any, cb) => {
      ...
      if (req.body.bucket === 'affiliate') {
        bucketName = config.awsConfig.AFFILIATE_BUCKET_FOLDER;
      }
    ...
    metadata: function (req: Request, file: any, cb) {
      cb(null, { fieldName: `_affiliate_file_${uuidv4()}_${path.extname(file.originalname)}` });
    },
    key: function (req: Request, file: any, cb) {
      cb(null, `_affiliate_file_${uuidv4()}_${path.extname(file.originalname)}`);
    },
  }),
});
